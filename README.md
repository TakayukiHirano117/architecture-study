...
| Lint | golangci-lint | - |
| ドキュメント | pkgsite | pkg.go.dev形式のパッケージドキュメント |
...
[pkgsite](https://pkg.go.dev/golang.org/x/pkgsite/cmd/pkgsite)でpkg.go.dev形式のパッケージドキュメントをローカルで表示できます。

```bash
# 初回のみ: pkgsiteをインストール
make install-docs

# ドキュメントサーバーを起動（ブラウザが自動で開く）
make docs
```

デフォルトで `http://localhost:8080` でサーバーが起動し、プロジェクトのパッケージと依存モジュールのドキュメントを閲覧できます。
