...
import (
	...
	contractrequestdm "github.com/TakayukiHirano117/architecture-study/src/core/domain/contract_requestdm"
...
func TestContractRequest_NewContractRequest(t *testing.T) {
	tests := []struct {
		name       string
		setupFunc  func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted)
		wantErr    bool
		errMsg     string
		assertions func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted)
	}{
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				return id, "プランに興味があります。よろしくお願いします。", menteeID, 5000, planID, contractrequestdm.Pending
			},
			wantErr: false,
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, strings.Repeat("あ", 500), menteeID, 5000, planID, contractrequestdm.Pending
			},
			wantErr: false,
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "a", menteeID, 5000, planID, contractrequestdm.Pending
			},
			wantErr: false,
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "メッセージ", menteeID, 3000, planID, contractrequestdm.Pending
			},
			wantErr: false,
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "メッセージ", menteeID, 1000000, planID, contractrequestdm.Pending
			},
			wantErr: false,
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "", menteeID, 5000, planID, contractrequestdm.Pending
			},
			wantErr: true,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, strings.Repeat("あ", 501), menteeID, 5000, planID, contractrequestdm.Pending
			},
			wantErr: true,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "メッセージ", menteeID, 2999, planID, contractrequestdm.Pending
			},
			wantErr: true,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				menteeID := shared.NewUUID()
				planID := shared.NewUUID()
				return id, "メッセージ", menteeID, 1000001, planID, contractrequestdm.Pending
			},
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cr, err := contractrequestdm.NewContractRequest(id, message, menteeID, priceAtRequest, planID, isAccepted)
...
func TestContractRequest_NewContractRequestByVal(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupFunc: func(t *testing.T) (shared.UUID, string, shared.UUID, uint32, shared.UUID, contractrequestdm.IsAccepted) {
				...
				return id, "契約申請メッセージ", menteeID, 10000, planID, contractrequestdm.Accepted
			},
			assertions: func(t *testing.T, cr *contractrequestdm.ContractRequest, id shared.UUID, message string, menteeID shared.UUID, priceAtRequest uint32, planID shared.UUID, isAccepted contractrequestdm.IsAccepted) {
				assert.NotNil(t, cr)
				assert.Equal(t, id, cr.ID())
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cr, err := contractrequestdm.NewContractRequestByVal(id, message, menteeID, priceAtRequest, planID, isAccepted)
			require.NoError(t, err)
...
func TestContractRequest_Getters(t *testing.T) {
	...
	planID := shared.NewUUID()
	isAccepted := contractrequestdm.Pending
